  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/95 backdrop-blur-md supports-[backdrop-filter]:bg-background/85">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-0.5 xl:gap-1 lg:flex" aria-label="Primary navigation">
          {groups.map((group) => (
            <div key={group.label} className="group relative">
              <button
                type="button"
                className="flex items-center gap-1 rounded-lg px-2 xl:px-3 py-2 text-xs xl:text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground group-hover:text-foreground whitespace-nowrap shrink-0"
                aria-haspopup="true"
              >
                {group.label}
                <ChevronDown className="h-3 w-3 xl:h-3.5 xl:w-3.5 transition-transform duration-200 group-hover:rotate-180 shrink-0" />
              </button>
          ...
          {directLinks.map(([label, to]) => {
            ...
            return (
              <Link
                key={to}
                to={to}
                className={`flex items-center gap-1.5 rounded-lg px-2 xl:px-3 py-2 text-xs xl:text-sm font-medium transition-colors hover:bg-muted hover:text-foreground whitespace-nowrap shrink-0 ${
                  isFeatured ? "bg-primary/10 font-semibold text-primary" : "text-muted-foreground"
                }`}
              >
                {label}
                {isFeatured && <span className="flex h-1.5 w-1.5 xl:h-2 xl:w-2 rounded-full bg-primary animate-pulse shrink-0" />}
              </Link>
            );
          })}
        </nav>
        ...
        <div className="hidden items-center gap-1.5 xl:gap-3 lg:flex shrink-0">
          <Link
            to="/search"
            className="flex items-center gap-1.5 rounded-xl border border-border/80 bg-muted/40 px-2 xl:px-3 py-1.5 text-xs text-muted-foreground transition-all hover:border-primary/40 hover:bg-muted/70 hover:text-foreground whitespace-nowrap shrink-0"
          ...
          <Link
            to="/work-with-us"
            className="inline-flex items-center gap-1.5 rounded-xl bg-primary px-3 py-1.5 xl:px-4 xl:py-2 text-xs xl:text-sm font-semibold text-primary-foreground shadow-xs transition-all hover:bg-primary/90 hover:shadow-md active:scale-[0.98] whitespace-nowrap shrink-0"
          >
