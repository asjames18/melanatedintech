...
After applying, regenerate the Supabase TypeScript types so the `as never` /
`as any` casts added this round can be tightened later (optional, not blocking):
...
Server env (host dashboard — Lovable Publish, later Vercel/CF):
...
- `LOVABLE_API_KEY` — for the Stripe-via-Lovable gateway client.
...
`STRIPE_SANDBOX_API_KEY` / `PAYMENTS_SANDBOX_WEBHOOK_SECRET` for test runs.)
