...
export const PREMIUM_CATALOG: Record<PremiumKind, Record<string, PremiumEntry>> = {
  agent: {
    ...
    // These four now use proper reusable Stripe Price objects (lookup keys
    // below) instead of inline price_data, so the amounts are managed in the
    // Stripe dashboard. Fulfillment still validates the settled amount against
    // amountCents, and the catalog stays the drift-proof reference.
    "marketing-campaign-strategist": {
      priceId: "agent_marketing_campaign_strategist_47_onetime",
      amountCents: 4700,
    },
    "pa-inbox-zero": { priceId: "agent_pa_inbox_zero_onetime", amountCents: 3900 },
    "marketing-seo-researcher": {
      priceId: "agent_marketing_seo_researcher_47_onetime",
      amountCents: 4700,
    },
    "personal-chief-of-staff": {
      priceId: "agent_personal_chief_of_staff_47_onetime",
      amountCents: 4700,
    },
    "customer-support-agent": {
      priceId: "agent_customer_support_agent_47_onetime",
      amountCents: 4700,
    },
  },
  product: {
