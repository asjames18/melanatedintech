...
function WebsiteLaunchNurtureAdmin() {
  ...
  return <SiteLayout>
    <PageHeader eyebrow="Protected marketing operations" title="Website Launch nurture" description="The four-email checklist sequence is paused by default. Activate only after reviewing the audience and approving the first send." actions={<Button asChild variant="outline"><Link to="/admin"><ArrowLeft className="h-4 w-4" /> Admin dashboard</Link></Button>} />
    <section className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Campaign status</p><p className="mt-2 text-2xl font-semibold">{enabled ? "Active" : "Paused"}</p><p className="mt-1 text-sm text-muted-foreground">Activation schedules the first email for consented checklist subscribers. No Beehiiv or external list is used.</p></div>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-3"><Metric label="Total enrollments" value={String(enrollments.length)} /><Metric label="Active" value={String(counts.active ?? 0)} /><Metric label="Completed / stopped" value={String((counts.completed ?? 0) + (counts.unsubscribed ?? 0) + (counts.suppressed ?? 0))} /></div>
      </div>
      <div className="mt-8 rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-semibold">Safety rules</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Every due message rechecks consent and the authoritative suppression table before entering the email queue. Unsubscribe changes the enrollment to unsubscribed, and the queue processor performs a second suppression check immediately before delivery. Operational contact notifications remain separate.</p></div>
    </section>
  </SiteLayout>;
}
